...
int searchCutNode(int *target_node, int* adjacency_a, int* adjacency_b, int length);
...
int getReturnFromAdj(int value, int* adjacency, int length, int*);
...
int* adjacency_a_index2;
int* adjacency_b_index2;
...
int calculateDCJdistance(int* adjacency_a, int* adjacency_b, int length)
{
    ...
    memcpy(copy_adj_a, adjacency_a, sizeof(int)*length);
    int ab_path = 0;
    int circle = 0;
    int st = EPTY;
    for (k = 0; k < length-1; k++) {
        st = EPTY;
        if (copy_adj_a[k] != VSIT)
            st = copy_adj_a[k];
        else continue;
        int val = 0;
        while (1)
        {
            if (st != TAIL) i = adjacency_a_index[st];
            else i = adjacency_a_index[length-1];
            val = 0;
            if (i%2 == 0)
            {
                next = copy_adj_a[i+1];
                if (copy_adj_a[i+1]==VSIT || copy_adj_a[i]==VSIT) {val = 1;}
                copy_adj_a[i+1] = VSIT;
                copy_adj_a[i] = VSIT;
            }
            else
            ...
            if (next != TAIL) i = adjacency_b_index[next];
            else i = adjacency_b_index[length-1];
            if (i%2 == 0)
            {
                st = adjacency_b[i+1];
            }
    ...
    return length/2 - circle - ab_path/2;
}
	...
	for (i=0; i<3; i++)
	{
		for (j=0; j<3; j++)
		{
			if (i!=j)
			{
				...
				int distance =  PD[i][j];//calculateDCJdistance(temp_initial_genome, original_list[j], length);
	...
	for(int i=0;i<popsize;i++)
	{
		for (int k = 0; k < DIMENSION; k++)
		{
			if(mbest[k]!=population[i].x[k])
			{
				...
				{
						//(1)find the index 锛宎nd save to swap_operator,then break
...
void quantumpso(int** original_list, int* median_genome, int length, int* bestMScore)
{
	...
	double emin, e1, e2, e3;
	srand((unsigned)time(NULL));
	sum=0;

	PD[0][1] = PD[1][0] = calculateDCJdistance(original_list[0], original_list[1], length);
    PD[0][2] = PD[2][0] = calculateDCJdistance(original_list[0], original_list[2], length);
    PD[2][1] = PD[1][2] = calculateDCJdistance(original_list[1], original_list[2], length);
    medianLB = (int) ceil((PD[0][1]+PD[0][2]+PD[1][2]) / 2);
    emin = medianLB*2.0;
    e1 = PD[0][1]+PD[0][2]-PD[1][2];
    e2 = PD[0][1]+PD[1][2]-PD[0][2];
    e3 = PD[1][2]+PD[0][2]-PD[0][1];

    MAXITER = GENERATION;
    if (emin < MAXITER) MAXITER = (int)emin;
    actualLoop = 0;
	...
	//set the initialize solution 
		...
		// 锛坅锛� the first time to print median score
        //printf("firstmedianScore=%d\n",minimum);
         // 锛坆锛塼he first print fitness(median score)
    ...
    //step2:the iteration process
		for (t = 0; t < MAXITER; t++)
		{
			...
			w=(1.0-0.5)*(MAXITER-t)/MAXITER+0.5;
				...
				*/
            /* 锛�1锛塩opmute the mbest accroding to the average of fitness,rather than the solution*/
				tmp_average=0;
				for (int i = 0; i < popsize; i++)
				{
					/* code */
			   ...
			   // 锛坆锛塼he second print fitness(median score)
			  /* for(i=0;i<popsize;i++)
			   {
			   	 medianScore[i]=calculateMedianScore(mbest, original_list, length);	
			     //printf("secondmedianScore2[%d]=%d\n",i,medianScore[i]);
			   }*/
			   // step2 for mbest锛歛verage between swarm[optmbest].x[k] and the better 50 percent 
			   tmp_average_sec=0;
			   temp_num=0;
				for (int i = 0; i < popsize; i++)
				{
			 ...
			 /*锛�2锛塩opmute the pid accroding to the swap operator*/								
				for(i=0;i<popsize;i++)
				{
					...
					//updateTrueGenome=gbest[k]
					 ...
					 memcpy(temp_sbestGenome,updateTrueGenome,sizeof(int)*length);
					...
					for (k=0;k<DIMENSION;k++)
					{
						...
						// 锛坈锛塼he third print fitness(median score)
                   ...
                   */
					 ...
					 /*锛�3锛塩opmute the swarm[i].x[k] accroding to the swap operator*/
					...
					// 锛坉锛塼he fourth print fitness(median score)
                ...
                actualLoop++;
				if (minimum == medianLB) break;
		}
		sum=sum+minimum;
		bestMedianScore=minimum;
		for (i=0; i<actualLoop; i++)
	   {
...
int calculateDCJdistance1(int* adjacency_a, int* adjacency_b, int length)
{
	int i;
	//record the index of adjacency
	for (i=0; i<length; i++)
...
int searchCutNode(int *target_node, int* adjacency_a, int* adjacency_b, int length)
{
	...
	while(count < length)
	{
		...
		if (temp_node[0] == TAIL) i = adjacency_b_index2[length-1];
                else  i = adjacency_b_index2[temp_node[0]];
		if (i%2 == 0)
		{
...
int checkNodeInCircle(int *cutNode, int* adjacency_a, int* adjacency_b, int length)
{
	...
	int returned_b1, returned_a1;
	step = 0;
	start_b = first;

	while(1)
	{
		returned_b = getReturnFromAdj(start_b, adjacency_b, length, adjacency_b_index2);
		step++;
		if (returned_b == HEAD || returned_b == TAIL) return -1;
		start_a = returned_b;
		returned_a = getReturnFromAdj(start_a, adjacency_a, length, adjacency_a_index2);
		step++;
		if (returned_b == HEAD || returned_b == TAIL) return -1;
...
int getReturnFromAdj(int value, int* adjacency, int length, int* adjacency_index)
{
	int i;
	if (value == TAIL) i = adjacency_index[length-1];
	else i = adjacency_index[value];
...
void splitBigCircleIntoSmall(int* adjacency_a, int* adjacency_b, int length, int *cutNode)
{
	...
	if (first_adj == TAIL) first_index = adjacency_a_index2[length-1];
	else first_index = adjacency_a_index2[first_adj];

	if (second_adj == TAIL) second_index = adjacency_a_index2[length-1];
	else second_index = adjacency_a_index2[second_adj];
...
void produceSmallCircle(int* target_node, int* adjacency_a, int* adjacency_b, int length)
{
	int i, fn_index, sn_index, temp, fn1, sn1;
	if (target_node[0] == TAIL) fn_index = adjacency_a_index2[length-1];
	else fn_index = adjacency_a_index2[target_node[0]];

	if (target_node[1] == TAIL) sn_index = adjacency_a_index2[length-1];
	else sn_index = adjacency_a_index2[target_node[1]];
...
void DCJOperation(int* adjacency_a, int* adjacency_b, int length)
{
	...
	for (i=0; i<length; i++)
	{
		if (adjacency_a[i] == TAIL)
		{
			adjacency_a_index2[length-1] = i;
		}
		else
		{
			adjacency_a_index2[adjacency_a[i]] = i;
		}
	}
	...
	for (i=0; i<length; i++)
	{
		if (adjacency_b[i] == TAIL)
		{
			adjacency_b_index2[length-1] = i;
		}
		else
		{
			adjacency_b_index2[adjacency_b[i]] = i;
		}
	}
	if (searchCutNode(target_node, adjacency_a, adjacency_b, length) < 0) return;//cannot find cut node after so many random searches
...
int main(int argc, char **argv)
{
	...
	*/
	...
	best_scorelist_qpso = (int *)malloc(GENERATION* sizeof(int));
	current_scorelist = (int *)malloc(GENERATION* sizeof(int));

	current_scorelist_qpso = (int *)malloc(GENERATION* sizeof(int));
	adjacency_a_index = (int *)malloc(length* sizeof(int));
	adjacency_b_index = (int *)malloc(length* sizeof(int));
    adjacency_a_index2 = (int *)malloc(length* sizeof(int));
    adjacency_b_index2 = (int *)malloc(length* sizeof(int));
	...
	data = (int *)malloc(GENERATION*sizeof(int));
 ...
 //    printf("optMScore: %d\n", optMScore);
	...
	*/
    ...
    free(adjacency_a_index2);
    free(adjacency_b_index2);
